...
def read_kafka_stream(spark, topic, schema):
    ...
    return (
        ...
        .select(from_json(col('value'), schema).alias('data'))  #Parses the JSON string in the 'value' field according to the specified schema and aliases the result as 'data'
        .select('data.*') #Selects all fields from the 'data' struct, flattening the structure.
        .withWatermark('timestamp', '2 minutes') # '2 minutes'): Adds a watermark to the data using the 'timestamp' column, allowing for late data to be processed but limiting how late it can be (2 minutes in this case)
    )
